...
export default function ListadoPrestamos() {
  ...
  const estadoPrest = async (est, id, row) => {
    await confirmAlert({
      title: "ATENCION",
            ...
            axios
              ...
              .then((res) => {
                if (res.status === 200) {
                  ...
                  registrarHistorialAprobacion(row, usu.usuario);
              ...
              .catch((error) => {
                console.log(error);
                toast.error(
                  "Ocurrio un error al actualizar el estado del subsidio"
                );
              });
          },
        },
      ...
      {!usu ? (
        ...
        <>
          {usu.perfil === 1 || usu.perfil === 3 ? (
            ...
            <PrestamosOp listado={listado} usu={usu} noData={noData} />
          ) : null}
        </>
      )}
    </>
