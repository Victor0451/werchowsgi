...
import FormSolicitudes from "@/components/socios/FormSolicitudes";
    ...
    await axios
      ...
      .then((res) => {
        if (res.data.length > 0) {
          let adh = JSON.parse(res.data);

          let lis = ficha.concat(adh);

          if (lis.length < 9) {
            let row = 9 - lis.length;
