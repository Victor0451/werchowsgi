...
export const FormAcciones = ({
    row,
    handleChange,
    errores
}) => {

    let cuoSinBonif = row.cuota * 1.10
    ...
    return (
        <>
            <div className=' grid gap-6 mb-6 md:grid-cols-3'>
                ...
                <Input size="md" label="Socio" defaultValue={`${row.apellido}, ${row.nombre}`} readOnly />
            ...
            <div className='mt-2 grid gap-6 mb-6 md:grid-cols-2 '>
                <Alert className=' text-center' color="blue"><strong><u>Cuota Bonificada</u></strong>: ${row.cuota}</Alert>
            ...
            <div className='mt-4 grid gap-6 mb-6 md:grid-cols-3'>
                ...
                <Input size="md" type={"date"} label="Fecha" onChange={(e) => { handleChange('fecha naccion', e.target.value) }} />

                <Textarea size="md" rows={3} label='Observacion' onChange={(e) => { handleChange('observacion nuac', e.target.value) }} />
            </div>
