...
export const FormAccionesTemp = ({ row, handleChange, errores, f }) => {
  let cuoSinBonif = row.IMPORTE * 1.1;
    ...
    <>
      <div className=" grid gap-6 mb-6 md:grid-cols-3">
        ...
        <Input
          size="md"
          label="Socio"
          defaultValue={`${row.APELLIDOS}, ${row.NOMBRES}`}
          readOnly
        />
      ...
      {f === "T" ? (
        <div className="border-2 mt-5 p-4 rounded-xl">
          <Typography variant="h5" color="blue-gray">
            Accion Registrada
          </Typography>

          <div className=" mt-5 grid gap-6 mb-6 md:grid-cols-3">
            <Input
              size="md"
              label="Fecha Observacion"
              defaultValue={moment(row.fecha_observacion).format("DD/MM/YYYY")}
              readOnly
            />
          </div>
          <Textarea
